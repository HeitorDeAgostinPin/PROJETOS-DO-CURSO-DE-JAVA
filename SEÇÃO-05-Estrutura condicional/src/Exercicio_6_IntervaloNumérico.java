import java.util.Scanner;

public class Exercicio_6_IntervaloNumérico {
//Intervalo Numérico – Determinar em qual intervalo um número se encontra.

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		//Você deve fazer um programa que leia um valor qualquer 
		//e apresente uma mensagem dizendo em qual dos 
		//seguintes intervalos ([0,25], (25,50], (50,75],
        //(75,100]) este valor se encontra. Obviamente se o valor não estiver em 
		//nenhum destes intervalos, deverá ser impressa a mensagem 
		//“Fora de intervalo”
		
		
		
		Scanner sc = new Scanner(System.in);
		
		
		System.out.println("Digite um numero:");
		int valor = sc.nextInt();
		
		if (valor >=0 && valor<=25) 
		{
			System.out.println("O valor está no intervalo [0 a 25]");
		} else if (valor >25 && valor<=50) 
		{
			System.out.println("O valor está no intervalo [25 a 50]");

		}else if (valor >50 && valor<=75) 
		{
			System.out.println("O valor está no intervalo [50 a 75]");

		}else if (valor >75 && valor<=100) 
		{
			System.out.println("O valor está no intervalo [50 a 75]");
	
		}else {
			System.out.println("O valor está Fora do Intervalo");

		}
		
		sc.close();
	}

}
